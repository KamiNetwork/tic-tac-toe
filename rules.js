...
function begin_play(){
	if (started == true) {
		alert("Game already start, please reset.");
		exit();
	}
	if (document.getElementById("player1_id").value == "" || document.getElementById("player2_id").value == "") {
		alert("One of the Player Name Empty, game could not start");
		exit();
	}
...
function reset_play(){
	...
	document.getElementById("player1_id").disabled="";
	document.getElementById("player2_id").value = "";
	document.getElementById("player2_id").disabled="";
...
function play() {
	...
	if(document.getElementById("move_text_id").value != "A1" && document.getElementById("move_text_id").value != "A2" && document.getElementById("move_text_id").value != "A3" && document.getElementById("move_text_id").value != "B1" && document.getElementById("move_text_id").value != "B2" && document.getElementById("move_text_id").value != "B3" && document.getElementById("move_text_id").value != "C1" && document.getElementById("move_text_id").value != "C2" && document.getElementById("move_text_id").value != "C3") {
		alert("INPUT ERROR: ONLY ACCEPT A1, A2, A3, B1, B2, B3, C1, C2, C3");
		exit();
	}
	var targetColo = document.getElementById("move_text_id").value;
	var stateColo = -1;
	if (targetColo == "A1") { stateColo = 0; }
	if (targetColo == "A2") { stateColo = 1; }
	if (targetColo == "A3") { stateColo = 2; }
	if (targetColo == "B1") { stateColo = 3; }
	if (targetColo == "B2") { stateColo = 4; }
	if (targetColo == "B3") { stateColo = 5; }
	if (targetColo == "C1") { stateColo = 6; }
	if (targetColo == "C2") { stateColo = 7; }
	if (targetColo == "C3") { stateColo = 8; }
	if (document.getElementById(targetColo).innerHTML == targetColo) {
		if (turn == 1) {
			board_state[stateColo] = 1;
			document.getElementById(targetColo).innerHTML = "X";
			document.getElementById("turn_info").innerHTML = "Turn For: <b> O </b>";
			turn = 0;
		} else {
			board_state[stateColo] = 0;
			document.getElementById(targetColo).innerHTML = "O";
			document.getElementById("turn_info").innerHTML = "Turn For: <b> X </b>";
			turn = 1;
		}
		
	} else {
		alert(targetColo + " is owned by " + document.getElementById(targetColo).innerHTML);
	}
